...
'''
	...
	def __upload_dir(self, localpath, remotepath, ondup = 'overwrite'):
		...
		for walk in os.walk(localpath, followlinks=self.__followlink):
			self.__walk_upload(localpath, remotepath + '/' + os.path.basename(localpath), ondup, walk)
...
options = {"push": push, "pull": pull, "rm": remove, "ls": listflags}
...
USAGE:
	...
	<script> pull [flag] - 下载所有标记为[flag]的文件
	<script> rm [flag] - 删除所有标记为[flag]的文件
	<script> ls <flag> - 显示所有flag/显示flag下的内容
'''
		sys.exit()
	options.get(sys.argv[1])(sys.argv[2:])
